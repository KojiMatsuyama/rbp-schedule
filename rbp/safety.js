...
// 散布記録(records)から使用回数・直近散布・ローテーション状態を算出する。
...
function systemCodesUsedOn(dateKey, records) {
  const ids = records[dateKey].pesticideIds || [];
...
function computeRotationState(records, targetDateStr, lookbackCount = 5) {
  ...
  const initialSystems = systemCodesUsedOn(mostRecentKey, records);
  const initialState = {
  ...
  const { rotationState } = earlierKeys.reduce(({ rotationState, activeSystems }, dateKey) => {
    const systemsUsed = systemCodesUsedOn(dateKey, records);
